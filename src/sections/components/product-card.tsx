...
const ProductCard = ({ product }: PRODUCT_CARD_PROPS) => {
    ...
    // Add fallback for media
    const productImage = product.media?.[0]?.link || '/images/default-product.png';
    ...
    return (
        <Card variant='outlined'>
            <CardActionArea LinkComponent={Link} href={`/product/${product.slug}`}>
                ...
                <CardContent>
                    <Stack gap={1}>
                        <Typography noWrap variant='h6' color='textPrimary' fontWeight={600}>{product?.name || 'Unnamed Product'}</Typography>
                        <Stack direction='row' gap={1} alignItems='center'>
                            <Rating size='small' readOnly value={product?.rating || 0} precision={0.1} />
                            <Typography variant='body2' color='textSecondary'>({0})</Typography>
                        </Stack>
                        <Typography color='textSecondary'>{product?.store?.name || 'Unknown Store'}</Typography>
                        <Stack direction='row' gap={1} alignItems='center'>
